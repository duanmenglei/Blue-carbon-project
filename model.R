...
metrics <- crown_metrics(cnlas,
                         ~list( 
                                ...
                                Hentropy = entropy(Z),
                                H01TH = quantile(Z,0.01),H02TH = quantile(Z,0.02),
                                H03TH = quantile(Z,0.03),H04TH = quantile(Z,0.04),
                                H05TH = quantile(Z,0.05),H06TH = quantile(Z,0.06),
                                H07TH = quantile(Z,0.07),H08TH = quantile(Z,0.08),
                                H09TH = quantile(Z,0.09),H10TH = quantile(Z,0.10),
...
# import data table
metrics <- read.csv("D:/UAlberta/PhD/Blue carbon project/forest inventory/model-.csv/model-train.csv")
attach(metrics)

set.seed(222)
ind <- sample(2, nrow(metrics), replace = TRUE, prob = c(0.8, 0.2))
train <- metrics[ind==1,]
test <- metrics[ind==2,]
...
RFmodel <- randomForest(Biomass_ob ~ Hmean+Hsd+Hmedian+Hmax+Hskew+Hkurt+Hentropy
                       
                       +H91TH+H92TH+H93TH+H94TH+H95TH+H96TH+H97TH+H98TH+H99TH,
...
ggplot(test, aes(x= Biomass_ob , y= predict_biomass))+
  geom_point(alpha=0.5)+
  geom_abline(slope = 1, linetype="dashed")+
  theme_bw()+
  labs(x="Observed biomass", y="Predicted biomass")+
...
RMSE = RMSE(test$Biomass_ob, test$predict_biomass)
RMSE
MAE = MAE(test$Biomass_ob, test$predict_biomass)
MAE
R2 = R2(test$Biomass_ob, test$predict_biomass)
R2
  

# evaluate the variable importance, 
## and according to the result, drop those variables that contribute less
## to improve the computational efficiency and remove redundant imformation
...
# eventually,
# the model is ready to predict biomass in unknown area with available LiDAR data
...
predict_newdata_biomass <- predict(RFmodel, newdata = newdata)
newdata$predict_biomass <- predict_newdata_biomass

# output final newdata.csv with predict_biomass
write.csv(newdata, "newdata.csv", col.names = TRUE)
